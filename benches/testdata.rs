...
fn generate_files(rng: &mut StdRng, num_files: usize) -> Vec<String> {
    ...
    while files.len() < num_files {
        ...
        // Top-level dir
        path_parts.push(TOP_DIRS[rng.random_range(0..TOP_DIRS.len())].to_string());

        // Middle dirs
        for _ in 1..depth {
            let dir = if rng.random_bool(0.6) {
                MID_DIRS[rng.random_range(0..MID_DIRS.len())]
            } else {
                LEAF_DIRS[rng.random_range(0..LEAF_DIRS.len())]
            };
            path_parts.push(dir.to_string());
        }

        // Filename
        let name = FILE_NAMES[rng.random_range(0..FILE_NAMES.len())];
...
fn generate_codeowners(
    ...
    while rule_count < num_rules {
        ...
        let pattern = if roll < 0.15 {
            ...
            let depth = rng.random_range(1..=parts.len().min(3));
            format!("/{}/", parts[..depth].join("/"))
        } else if roll < 0.50 {
            // Deep wildcard
            let top = TOP_DIRS[rng.random_range(0..TOP_DIRS.len())];
            let ext = EXTENSIONS[rng.random_range(0..EXTENSIONS.len())];
            format!("{}/**/*.{}", top, ext)
        } else if roll < 0.65 {
            // Anchored directory
            let top = TOP_DIRS[rng.random_range(0..TOP_DIRS.len())];
            let mid = MID_DIRS[rng.random_range(0..MID_DIRS.len())];
            format!("/{}/{}/", top, mid)
        } else if roll < 0.75 {
            ...
            // No-match pattern
            format!("/nonexistent-{}/deep/path/", rng.random_range(0..10000))
        };

        // Generate owners for this rule
        let num_owners = if rng.random_bool(0.02) {
        ...
        if rng.random_bool(0.03) && !rule_owners.is_empty() {
